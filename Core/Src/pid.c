...
uint8_t PID_Compute(PID_TypeDef *uPID)
{
	...
	if (timeChange >= uPID->SampleTime)
	{
		...
		uPID->OutputSum     += (uPID->Ki * error);         //积分
		...
		if (uPID->POnE) output = uPID->Kp * error;        //比例
		else output = 0;
		
		/* ..... Compute Rest of PID Output ..... */
		output += uPID->OutputSum - uPID->Kd * dInput;    //微分
		...
		*uPID->MyOutput += output;
		...
		if (*uPID->MyOutput > uPID->OutMax) *uPID->MyOutput = uPID->OutMax;
		else if (*uPID->MyOutput < uPID->OutMin) *uPID->MyOutput = uPID->OutMin;
		return _TRUE;
...
void PID_SetMode(PID_TypeDef *uPID, PIDMode_TypeDef Mode)
{
	...
	if (newAuto && !uPID->InAuto) {
		...
		if (uPID->OutputSum > uPID->OutMax) uPID->OutputSum = uPID->OutMax;
		if (uPID->OutputSum < uPID->OutMin) uPID->OutputSum = uPID->OutMin;
	}
...
void PID_SetOutputLimits(PID_TypeDef *uPID, float Min, float Max)
{
	...
	if (uPID->InAuto) {
		/* ..... Check out value ..... */
		if (*uPID->MyOutput > uPID->OutMax) *uPID->MyOutput = uPID->OutMax;
		if (*uPID->MyOutput < uPID->OutMin) *uPID->MyOutput = uPID->OutMin;
 ...
 * ki：微分系数
 * kd：积分系数
 * output_limit_min：下限值
 * output_limit_max：上限值
 */
...
{
	/* ~~~~~~~~~~ Set parameter ~~~~~~~~~~ */
	PID(uPID, Temp, PIDOut, TempSetpoint, kp, ki, kd, _PID_P_ON_E, _PID_CD_DIRECT);
	PID_SetMode(uPID, _PID_MODE_AUTOMATIC);
	PID_SetSampleTime(uPID, SampleTime);
	PID_SetOutputLimits(uPID, output_limit_min, output_limit_max);
}
